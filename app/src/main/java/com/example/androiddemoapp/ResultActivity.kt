...
class ResultActivity : AppCompatActivity(), SubmitDataCallback , ContextAwareSigningCallback {
    ...
    lateinit var assentifySdk: AssentifySdk;
    ///
    ...
    override fun onCreateUserDocumentInstance(userDocumentResponseModel: CreateUserDocumentResponseModel) {
        ...
        contextAwareSigning.signature(userDocumentResponseModel.templateInstanceId,userDocumentResponseModel.documentId,"Base64 ..")
    }
    ...
    override fun onSignature(signatureResponseModel: SignatureResponseModel) {
        ...
        val outputPropertiesFaceModel = OutputPropertiesModelFace.getOutputPropertiesModel();
        ...
        var sharedStepDefinitionsSignature: SubmitRequestModel? = null;
        ...
        steps!!.forEach { item ->
            ...
            if (item.stepDefinition == StepsName.ContextAwareSigning) {
                ...
                item.outputProperties.forEach { property ->
                    if (property.key.contains(ContextAwareSigningKeys.DocumentURL)) {
                        values.put(property.key, signatureResponseModel.signedDocumentUri)
                    }
                }
                sharedStepDefinitionsSignature = SubmitRequestModel(
                    item.stepId, StepsName.ContextAwareSigning,
                    values
                );
        ...
        Log.e("Submit Data" ,listData.toString() )

        assentifySdk.startSubmitData(
            this, /// This activity implemented from from SubmitDataCallback
            listData  //  Your data
        )
        /** End **/
